...
function InputField({
  ...
  return (
    <div>
      <label className="block text-[13px] font-semibold mb-1.5" style={{ color: "var(--lp-fg)" }}>
        {label} {required && <span style={{ color: "var(--lp-accent)" }}>*</span>}
      </label>
      <div className="relative">
        <Icon size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: "var(--lp-muted)" }} />
        <input
          type={type}
          required={required}
          value={value}
    ...
    <AuthLayout>
      ...
      <div className="flex mb-8 rounded-xl overflow-hidden" style={{ border: "1px solid var(--lp-border)" }}>
        <button
          ...
          style={{
            background: tab === "login" ? "var(--lp-fg)" : "transparent",
            color: tab === "login" ? "white" : "var(--lp-muted)",
          }}
        >
          登录
        </button>
        <button
          ...
          className="flex-1 py-2.5 text-center text-sm font-semibold transition-all"
          style={{
            background: tab === "register" ? "var(--lp-fg)" : "transparent",
            color: tab === "register" ? "white" : "var(--lp-muted)",
          }}
        >
          注册
        </button>
      ...
      <div className="flex gap-3">
        <button className="flex-1 h-11 rounded-xl text-sm font-semibold flex items-center justify-center gap-2"
      ...
      <p className="text-center text-xs mt-6 leading-relaxed" style={{ color: "var(--lp-muted)" }}>
        {tab === "login" ? "登录" : "注册"}即表示你同意<a href="#" className="font-medium" style={{ color: "var(--lp-accent)" }}>用户协议</a>和<a href="#" className="font-medium" style={{ color: "var(--lp-accent)" }}>隐私政策</a>
      </p>
    </AuthLayout>
  );
}
